...
(*  Lemma Lab_var_dec :
  ...
  Definition sem_step l k
    := option_chain eff (hd_error l) = Some k.

  Inductive Tr (S : State -> Prop) : list Conf -> Prop :=
  | Init : forall s, S s -> Tr S [(root, start_tag, s)]
  | Step : forall l (k : Conf), Tr S l -> sem_step l k -> Tr S (k :: l).

  Definition EPath := Path eval_edge.

  (* this is a simplicification, as we don't use particular sets of initial states *)
(*  Inductive HyperTr (Σ : (State -> Prop) -> Prop) : (list Conf -> Prop) -> Prop :=
  | HyperInit : forall S, Σ S -> HyperTr Σ (fun l => exists s, S s -> l = [(root, start_tag, s)])
  | HyperStep : forall L (k : Conf), HyperTr Σ L -> HyperTr Σ (Tr (fun _ => False)).*)
  Inductive HyperTr (Σ : (State -> Prop) -> Prop) : (list Conf -> Prop) -> Prop :=
  | HyperInit : forall S, Σ S -> HyperTr Σ (fun l => exists s, S s -> l = [(root, start_tag, s)])
  | HyperStep : forall L l (k : Conf), HyperTr Σ L -> L l -> sem_step l k -> HyperTr Σ (fun l' => l = l' \/ L l).

  (* alternatively: *)
  Definition Tr' (S : State -> Prop) : list Conf -> Prop
    := fun t => exists s c, S s /\ EPath (root,start_tag,s) c t.

  (* but hypertraces cannot be encoded in this way, because Paths are predicates on lists, 
     while hypertraces operate on sets of traces *)

  (* EVERYTHING beyound this point must be redone *)
