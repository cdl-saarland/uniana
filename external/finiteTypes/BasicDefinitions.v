...
Ltac deq x := destruct (decision (x=x)) as [[]  | isnotequal]; [> | contradict isnotequal; reflexivity] .
...
Lemma injectiveApply (X Y: Type) (f: X -> Y) (x y: X) :
  injective f -> f x = f y -> x = y.
Proof.
auto.
Qed.

Lemma surjectiveApply (X Y: Type) (f: X -> Y) (x y: X) :
  surjective f -> forall y, exists x, f x = y.
Proof.
  auto.
Qed.

Lemma loop (X Y: Type) (f: X -> Y) (g: Y -> X) :(forall x, g(f x) = x) -> injective f.
Proof.
  intro H. unfold injective. intros x  y G. pose proof (H x) as H'. specialize (H y). rewrite G in H'.
     subst x. exact H.
Qed.
...
Definition pure (X:Type) (p: X -> Prop) {D:forall x, dec (p x)} x := if decision (p x) then True else False.
...
Lemma pure_impure  (P: Prop) (_: dec P) (norm: if decision (P) then True else False) : P.
Proof.
  dec; tauto.
Qed.
...
Proof.
  pose (f := fun y: X => fun E: x = y => match (decision (x = y)) with                                   
                                   | left b => b
                                   | _ => E end).
...
Lemma DecRef (X: eqType) (x:X) : decision (x = x) = left eq_refl.
Proof.
  dec.
...
(** * Decidablility of equality: Instance declarations *)
 (** Basic Equality types are declared as instances of dec.
Note that the corresponding lemma for bool is already definied in the base library/ External.v

*)
Instance unit_eq_dec :
  eq_dec unit.
Proof.
  intros x y. left. now destruct x ,y.
Qed.
...
(** The equality of cartesian products, option and sum types of two types with decidablility of equality are defined as instances of dec. *)

Instance prod_eq_dec (T1 T2: Type) (E1: eq_dec T1) (E2: eq_dec T2):
  eq_dec (T1 * T2).
Proof.
  intros. unfold dec.  destruct x, y. decide (t = t1); decide (t0 = t2);  left + right; congruence.
Qed.

Instance option_eq_dec (X: Type) (e: eq_dec X) :
  eq_dec (option X).
Proof.
  intros. destruct x, y;  try (left + right; congruence).
  - decide (x = x0); left + right; congruence.
Qed.

Instance sum_eq_dec (X: Type) (Y: Type) (ex: eq_dec X) (ey: eq_dec Y):
  eq_dec (sum X Y).
Proof.
  intros. destruct x,y; try (right; congruence).
  - decide(x = x0); [> left |right ]; congruence.
  - decide(y0 = y); [> left | right]; congruence.
Qed.
...
(** This function counts the number of occurences of an element in a given list and returns the result *)
  Fixpoint count {X: eqType}  (A: list  X) (x:  X) {struct A} : nat :=
    match A with
    | nil => O
    | cons y A' =>  if decision (x=y) then S(count A' x) else count A' x end.
...
Proof.
  ...
  -  rewrite <- H. destruct B.
    ...
    + exfalso. eapply app_cons_not_nil. now rewrite H.
- destruct A; auto. discriminate  H.
Qed.
...
Proof.
  ...
  - cbn. dec.
    ...
    + intros H G. apply IHA; auto. destruct G; congruence.
Qed.   

Lemma negOr A B: ~ (A \/ B) -> ~ A /\ ~ B.
Proof.
  tauto.
Qed.


Lemma consAppend (X: Type) (A: list X) (x: X) :
  cons x A = [x] ++ A.
Proof.
  reflexivity.
Qed.
...
Proof.
  intros [A B].  induction A;  cbn; omega.
Qed.

Lemma lengthEq (X: Type) (A B: list X):
  A = B -> |A| = |B|.
Proof.
congruence.
Qed.
